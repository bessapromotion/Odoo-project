...
class DemandeAchat(models.Model):
    ...
    def _get_default_location(self):
        loc = self.env['stock.location'].search(
            [('usage', '=', 'internal'), ('name', '=', 'Stock'), ('location_id', '=', 11)])
        if loc.exists():
            return loc[0].id
        else:
            return None
    ...
    employe_id = fields.Many2one('hr.employee', 'Demandeur', required=True, readonly=1,
                                 states={'new': [('readonly', False)]}, tracking=True)
    employe_glb_id = fields.Many2one('hr.employee', 'Demandeur')
    department_id = fields.Many2one('hr.department', 'Service demandeur', required=True, readonly=1,
                                    states={'new': [('readonly', False)]}, tracking=True)
    department_glb_id = fields.Many2one('hr.department', 'Service demandeur')
    project_id = fields.Many2one('project.project', string='Projet',
                                 states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    ddate = fields.Date('Date demande', default=date.today(), required=True, readonly=1,
                        states={'new': [('readonly', False)]}, tracking=True)
    location_id = fields.Many2one('stock.location', string='Stock', default=_get_default_location,
                                  states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    ...
    destination = fields.Selection([('demandeur', 'Livrer au demandeur'),
                                    ...
                                    ], string='Destination', default='demandeur',
                                   help=u'Quelle est la destination des produits demandés',
                                   states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    ...
    state = fields.Selection([('new', 'Nouvelle'),
                              ...
                              ('service', 'Service'),
                              ('done', u'Traitée'),
                              ('cancel', u'Annulée')], string='Etat', default='new', tracking=True)
    line_ids = fields.One2many('demande.achat.line', 'demande_id', string='Lignes demande', readonly=1,
                               states={'new': [('readonly', False)], 'stock': [('readonly', False)]})
    line_service_ids = fields.One2many('demande.achat.service.line', 'demande_id', string='Lignes demande', readonly=1,
    ...
    type_demande = fields.Selection([('achat', 'Stockable'), ('service', 'Service')], string='Type demande',
                                    required=True, default='achat', readonly=1, states={'new': [('readonly', False)]})
    company_id = fields.Many2one('res.company', string=u'Société', index=True,
    ...
    def action_verification_stock(self):
        ...
        if ready_for_verif_stock_ok:
            ...
            raise UserError(_(
                u'Certains produits ne sont pas validés ou annulés, veuillez les completer avant d\'envoyer pour la verification stock'))
    ...
    def get_picking_id(self, type_operation):
        picking = self.env['stock.picking'].search(
            [('picking_type_id', '=', type_operation.id), ('demande_id', '=', self.id), ])
        if picking.exists():
            return picking[0].id
        else:
            res_model, loc_id = self.env['ir.model.data']._xmlid_to_res_id('demande_achat.location_mad')
        ...
        if self.type_demande == 'achat':
            for rc in self.line_ids:
                if rc.state in ('new', 'waiting', 'valid'):
                    raise UserError(
                        _(u'Veuillez positionner l\'état de disponibilité des produits avant de créer les documents'))
                else:
                    if rc.state not in ('cancel', 'refus', 'done'):
                        if not rc.product_id:
                            ...
                            if rc.state in ('error',):
                                raise UserError(_(
                                    u'Le produit de la ligne (erreur) n\'a pas d\'emplacement stock par défaut. Veuillez verifier cette information dans la catégorie du produit, puis revérifier sa disponibilité'))
                            else:
...
class DemandeAchatLine(models.Model):
    ...
    description = fields.Char('Description de la demande', required=False)
    ...
    def create(self, values):
        ...
        #     values.update(product_id=False, description=False, qty=0, uom_id=False)

        # values.update(self._prepare_add_missing_fields(values))
        line = super(DemandeAchatLine, self).create(values)
    ...
    def action_disponible(self):
        if self.product_id:
            ...
            raise UserError(
                _('Veuillez sélectionner le produit en question avant de positionner la ligne sur DISPONIBLE'))
    ...
    def action_a_acheter(self):
        if self.product_id:
            ...
            raise UserError(_(
                u'Veuillez soit sélectionner le produit s\'il existe dans le catalogue produit, sinon le créer dans le cas contraire'))
