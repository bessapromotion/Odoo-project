...
class PurchaseOrder(models.Model):
    ...
    timbre         = fields.Monetary(string='Timbre', store=True, readonly=True,
                             compute='_amount_timbre', tracking=True)
    amount_timbre  = fields.Monetary(string='Total avec Timbre', store=True,
                                    readonly=True, compute='_amount_timbre', tracking=True)
...
class AccountMove(models.Model):
    ...
    # @api.one
    @api.depends('invoice_payment_term_id','amount_total')
    def _amount_timbre(self):
        for order in self:
            amount_timbre = order.amount_total
            if order.invoice_payment_term_id and order.invoice_payment_term_id.payment_type == 'cash':
               timbre = self.env['config.timbre']._timbre(order.amount_total)
               self.timbre = timbre['timbre']
               amount_timbre = timbre['amount_timbre']
            self.amount_timbre = amount_timbre
    ...
    def onchange_payment_term(self):
        if not self.invoice_payment_term_id:
            ...
            return
        values = {
            'payment_type': self.invoice_payment_term_id and self.invoice_payment_term_id.payment_type or False,
        }
        self.update(values)
