...
class CrmEcheancier(models.Model):
    ...
    @api.depends('payment_ids', 'payment_ids.state', 'montant_prevu')
    def _total_paiement(self):
        for rec in self:
            # total = 0.0
            if rec.type != 'remboursement':
                # total = sum(l.total for l in rec.payment_ids if l.state == 'posted')
                # for line in rec.payment_ids:
                #     if line.state == 'posted':
                #         total += line.amount
                # rec.montant = total
                rec.montant = sum(l.amount for l in rec.payment_ids if l.state == 'posted')
                rec.montant_restant = rec.montant_prevu - rec.montant
    ...
    amount_total_signed = fields.Monetary(related='order_id.amount_total', string='Total a payer',
    ...
    charge_recouv_id = fields.Many2one(related='order_id.charge_recouv_id', readonly=1, store=True,
                                       string=u'Recouvrement')

    date_creation = fields.Date('Date', required=1, readonly=1, states={'draft': [('readonly', False)]},
                                default=date.today())
    date_prevue = fields.Date('Date de paiement prévue', required=1, tracking=True, readonly=1,
                              states={'draft': [('readonly', False)]})
    date_paiement = fields.Date('Date de paiement', tracking=True, readonly=1,
                                states={'draft': [('readonly', False)]})

    # taux          = fields.Float('Taux', track_visibility='onchange', readonly=1, states={'draft': [('readonly', False)]})
    montant_prevu = fields.Monetary('Montant prevu', tracking=True,
                                    readonly=1)  # , states={'draft': [('readonly', False)]})
    montant = fields.Monetary(compute=_total_paiement, string=u'Montant payé', tracking=True, readonly=1,
                              store=True)
    montant_restant = fields.Monetary(compute=_total_paiement, string=u'Reste à payer', store=True, readonly=1,
                                      tracking=True)
    ...
    type = fields.Selection(
        [('tranche', 'Tranche'), ('tva', 'TVA'), ('notaire', 'Notaire'), ('remboursement', 'Ajustement Remboursement')],
        string='Type', default='tranche', readonly=True)
    ...
    def action_send_to_payment(self):
        ...
        view_id = self.env['ir.model.data']._xmlid_to_res_id('crm_echeancier.print_ordre_paiement_wizard_form_view')
    ...
    def wizard_change_amount(self):
        ...
        lst = self.env['crm.echeancier'].search(
            [('order_id', '=', self.order_id.id), ('state', 'in', ('draft', 'open'))])
        ...
        view_id = self.env['ir.model.data']._xmlid_to_res_id('crm_echeancier.echeance_amount_wizard_form_view')
