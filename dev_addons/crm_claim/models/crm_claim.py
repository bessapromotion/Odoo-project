...
class CrmClaim(models.Model):
    ...
    user_id = fields.Many2one('res.users', 'Responsible', tracking=True, default=lambda self: self.env['res.users'].browse(self._context['uid']))
    ...
    stage_id = fields.Many2one ('crm.claim.stage', 'Stage', tracking=True,
    ...
    state = fields.Selection([('valid', 'Valide'),
                              ('cancel', 'Annulée'), ], string='Etat', default='valid', tracking=True)
