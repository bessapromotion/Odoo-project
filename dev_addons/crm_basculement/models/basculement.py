...
class CrmBasculement(models.Model):
    ...
    def create_echeance(self, amount, e_date):
        e = self.env['crm.echeancier'].create({
            'name': '#Basculement',
            'order_id': self.b_order_id.id,
    ...
    def action_validate(self):
        ...
        for rec in self.affectation_ids:
            ...
            if rec.name.type_id != rec.basculement_id.order_id.order_line[0].product_id.type_id:
                raise UserError(_("Impossible de basculer d'une résarvation de type %s vers un devis de type %s",
                                  rec.basculement_id.order_id.order_line[0].product_id.type_id.name,
                                  rec.name.type_id.name)
                                )
            else:
                pass
        ...
        if self.prix_vente == self.b_price_2:
            ...
            ech_lst = self.env['crm.echeancier'].search([('order_id', '=', self.order_id.id)])
            for rec in ech_lst:
            ...
            if total_echeance > self.b_price_2:
                ...
                # ech_lst_cancel = self.env['crm.echeancier'].search([('order_id', '=', self.order_id.id)])
                for rec in ech_lst:
                    if rec.montant == 0:
                        rec.order_id = self.b_order_id.id
                        rec.state = 'cancel'
                    if rec.state == 'open' and rec.montant > 0:
                        rec.state = 'done'
            ...
            if total_echeance < self.b_price_2:
                ech_lst_reste = self.env['crm.echeancier'].search(
                    [('order_id', '=', self.order_id.id), ('state', '!=', 'done')])
                for rec in ech_lst:
                    rec.order_id = self.b_order_id.id
                    if rec.state != 'done':
                        if rec.state != 'cancel' and rec.montant > 0:
                            rec.state = 'done'
                            rec.montant_prevu = rec.montant
                            last_ech_date = rec.date_prevue
                        else:
...
class CrmBasculementAffectation(models.Model):
    ...
    name = fields.Many2one('product.template', string='Appartement', required=True,
                           domain="[('project_id', '=', project_id),('etat', 'in', ['Libre'])]")
    ...
    charge_recouv_id = fields.Many2one(related='basculement_id.charge_recouv_id', string='Chargé de recouvrement',
                                       readonly=1)
