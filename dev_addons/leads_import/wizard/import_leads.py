...
def _check_not_null(sh, row, col, name_field):
    ...
    if value and value != '':
        ...
        raise UserError(_(
            'Erreur a la ligne ' + str(
                row + 1) + ', Le champs (' + name_field + ') est vide, veuillez corriger sur le fichier excel et relancer l\'importation'))
...
class ImportLeadsWizard(models.TransientModel):
    ...
    # lead
    model2_id = fields.Many2one('leads.import.model', string='Modele d\'importation', required=False,
                                states={'lead': [('required', True)]})
        ...
        if field_val:
            ...
            if not self.elem_exist_req(model, nfield, field_val):
                ...
                msg = 'Erreur a la ligne ' + str(
                    row + 1) + ', Le ' + name_field + ' [' + _supp_dot_0(
                    field_val) + '] n\'existe pas sur la base Odoo, veuillez corriger sur le fichier excel ou créer cet élément puis relancer l\'importation'
                if not print_rep:
    ...
    def action_import_lead(self):
        ...
        def verify_data(sh):
            for row in range(1, sh.nrows):
                ...
                self.check_exist(sh, row, MODEL_OWNER, 'res_users', 'hubspot', 'HubSpot Owner name', fid,
                                 self.print_report)
        ...
        def create_partner(sheet):
            ...
            if not partner_id:
                ...
                client = self.env['res.partner'].create({
                    ...
                    'display_name': _get_cell(sheet, row_index, MODEL_NOM) + ' ' + _get_cell(sheet, row_index,
                                                                                             MODEL_PRENOM),
        ...
        if self.error:
            if self.print_report:
                raise UserError(_(
                    'Fichier contient des anomalies, veuillez consulter le fichier log généré [erreur_importation.csv]'))
        else:
            if self.cntrl_only:
    ...
    def action_import_crm(self):
        ...
        def verify_data(sh):
            for row in range(1, sh.nrows):
                ...
                # _check_exist(sh, row, MODEL_TIERS, 'res_partner', 'code_tiers', 'Tiers', self.print_report)
                # _check_not_null(sh, row, MODEL_COMPTE, 'Compte')
                self.check_exist(sh, row, MODEL_RECOUV, 'res_users', 'login', 'Chargé du recouvrement', fid,
                                 self.print_report)
        ...
        if self.error:
            ...
            for row_index in range(1, xsheet.nrows):
                ...
                partner_id = self.env['res.partner'].browse(
                    self.get_field_id(xsheet, row_index, MODEL_PARTNER, 'res.partner', 'display_name'))
                bien_id = self.env['product.product'].browse(
                    self.get_field_id(xsheet, row_index, MODEL_NAME, 'product.product', 'name'))
    ...
    def action_import_payment(self):
        ...
        def verify_data(sh):
            for row in range(1, sh.nrows):
                ...
                self.check_exist(sh, row, MODEL_MODE, 'mode_paiement', 'name', 'Mode de paiement', fid,
                                 self.print_report)
        ...
        def create_ordre_paiement(sheet, ech):
            mode = self.env['mode.paiement'].browse(
                self.get_field_id(xsheet, row_index, MODEL_MODE, 'mode.paiement', 'name'))
            ...
            if mode.name != 'Espece':
                ...
                doc_id = None
            com = self.env['res.users'].search(
                [('login', '=', _get_cell(sheet, row_index, MODEL_COMMERCIAL)), ])
