    ...
    def action_validate(self):
        ...
        if self.order_id.state not in ('sale', 'done',):
            ...
            if amount_paid < new_mtn_min_reserv :
                raise UserError(_('Le client a payé un montant de %s dzd qui est inférieur au seuil de validation de réservation (%s dzd)'%(amount_paid, new_mtn_min_reserv)))
            else:
    ...
    def onchange_devis(self):
        if self.order_id:
            ...
            for rec in self.order_id.order_line:
                if rec.product_id.default_code != '000000':
                    lines.append({
...
class CrmReservationProduct(models.Model):
    ...
    project_id = fields.Many2one(related='name.project_id', string='Projet', readonly=1)
    ...
    prix_m2 = fields.Float('Prix_m2', readonly=1)
    price_2 = fields.Float('Prix de vente', readonly=1)
