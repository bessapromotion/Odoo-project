...
class CrmVisite(models.Model):
    ...
    badge          = fields.Char(u'Numéro Badge', required=True)
    ...
    observation    = fields.Char('Observation', track_visibility='onchange')
    heure_entree   = fields.Datetime(u'Heure entrée', required=True, track_visibility='onchange')
    heure_recu     = fields.Datetime('Recu à ', track_visibility='onchange')
    heure_sortie   = fields.Datetime('Heure sortie', track_visibility='onchange')
    obs_hubspot    = fields.Char('Observation HUBSPOT', track_visibility='onchange')
    tags_ids       = fields.Many2many('visite.tags', string='Etiquettes')
    opportunity_id = fields.Many2one(comodel_name='crm.lead', string=u'Opportunité', track_visibility='onchange', readonly=1)
    ...
    state = fields.Selection([('valid', u'Effectuée'),
                              ('cancel', u'Annulée'), ], string='Etat', default='valid', track_visibility='onchange')
    ...
    def action_button_convert2opportunity(self):
        if self.observation:
            ...
            for visite in self:
                ...
                return opportunity_dict[visite.id].redirect_opportunity_view()
            return opportunity_dict
        else:
