...
class CrmPhonecall(models.Model):
    ...
    user_demand = fields.Many2one('res.users', string='Commercial demandé')
    user_charge = fields.Many2one('res.users', string='Commercial chargé', required=True)
    partner_id = fields.Many2one(
        comodel_name='res.partner',
        string='Client',
    )
    observation = fields.Char(string = 'Observation')
    etat = fields.Selection(related='partner_id.etat', string='Statut')
    ...
    partner_phone = fields.Char(string=u'Téléphone')
        ...
        for phonecall in self.filtered('date'):
            if phonecall.duration <= 0:
                duration = datetime.now() - datetime.strptime(
                    str(phonecall.date), DEFAULT_SERVER_DATETIME_FORMAT)
