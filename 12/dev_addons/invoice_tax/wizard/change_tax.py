...
class ChangeTaxWizard(models.TransientModel):
    ...
    name        = fields.Many2one('account.invoice', string='Facture', required=1, readonly=1, default=lambda self: self.env.context['active_id'])
    ...
    def action_appliquer(self):
        ...
        # table invoice taxes
        tax = self.env['account.invoice.tax'].search([('invoice_id', '=', self.name.id), ('tax_id', '=', self.tva_id.id)])
        if tax.exists():
