...
class CreerPaiementOutWizard(models.TransientModel):
    ...
    name        = fields.Many2one('crm.remboursement', string='Remboursement', required=1, readonly=1, default=lambda self: self.env.context['active_id'])
    ...
    def action_appliquer(self):
        if self.amount <= self.name.montant_restant:
            ...
            raise UserError(_('Le montant payé dépasse le reste a rembourser ! \n\n  Veuillez reverifier les informations du paiement !'))
