...
class CrmDesistement(models.Model):
    ...
    commercial_id = fields.Many2one('res.users', required=1, string='Commercial', readonly=1, states={'desistement': [('readonly', False)]})
    ...
    state     = fields.Selection([('desistement', u'Désistement déposé'),
                                  ...
                                  ('done', u'Terminé'),
                                  ('cancel', 'Annulée'), ], string='Etat', default='desistement', track_visibility='onchange')
    ...
    def create(self, vals):
        req = "select max(name) as num from crm_desistement where date_part('year', date)=%s;"
        self._cr.execute(req, (datetime.now().year,))
        res = self._cr.dictfetchall()
        num = res[0].get('num')
        if not num:
            num = 'DES/' + str(datetime.now())[2:4] + '/0000001'
        else:
            tmp = num[7:]
            vtmp = int(tmp) + 1
            num = 'DES/' + str(datetime.now())[2:4] + '/' + "{0:0>7}".format(str(vtmp))

        vals['name'] = num

        return super(CrmDesistement, self).create(vals)
    ...
    def create_dossier(self):
        ...
        i = 1
        for rec in dossier:
            doss = self.env['crm.desistement.dossier.line'].search([('dossier_id', '=', rec.id), ('desistement_id', '=', self.id)])
            if not doss.exists():
    ...
    def action_print(self):
        return self.env.ref('crm_operation.act_report_annulation').with_context({'discard_logo_check': True}).report_action(self.annulation_id.id)
