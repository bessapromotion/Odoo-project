...
class CreateDocumentWizard(models.TransientModel):
    ...
    destination  = fields.Selection([('demandeur', 'Livrer au demandeur'),
                                    ...
                                    ('entrepot', u'Transfert vers un dépôt')], string='Destination', help=u'Quelle est la destination des produits demandé')
    stock_destination_id = fields.Many2one('stock.location', string='Stock destination')
    methode      = fields.Selection([('consultation', 'Consulter plusieurs fournisseurs'), ('fournisseur', 'Consulter un fournisseur particulier')], string=u'Méthode de consultation', default='consultation')
    methode_req  = fields.Selection([('new', 'Nouvelle consultation'), ('old', 'Ajouter a une consultation en cours de préparation')], default='new', string='Nature de la consutation')
    ...
    def get_type_operation(self, location, destination='entrepot'):
        type_move = None
        if destination == 'entrepot':
            type_move = self.env['stock.picking.type'].search([('code', '=', 'internal'), ('mise_a_dispo', '=', 'no'), ('barcode', '=', 'WH-INTERNAL')])
        elif destination == 'demandeur':
    ...
    def get_picking_id(self, type_operation, destination='demandeur'):
        picking = self.env['stock.picking'].search([('picking_type_id', '=', type_operation.id), ('demande_id', '=', self.demande_id.id), ])
        if picking.exists():
            return picking[0].id
        else:
            loc_id = None
            ...
            elif destination == 'entrepot':
                loc_id = self.stock_destination_id.id
    ...
    def action_create_doc_affectation2(self):
        for rc in self.demande_id.line_ids:
            if (rc.qty_dispo > 0 or rc.qty_achat > 0) and rc.state in ('to_purchase', 'dispo'):
                res_model, loc_id = self.env['ir.model.data'].get_object_reference('demande_achat', 'location_mad')
                type_operation = self.get_type_operation(rc.location_id.id, 'demandeur')
                if type_operation:
                    picking = self.get_picking_id(type_operation, 'demandeur')
    ...
    def action_create_doc_transfert(self):
        for rc in self.demande_id.line_ids:
            if (rc.qty_dispo > 0 or rc.qty_achat > 0) and rc.state in ('to_purchase', 'dispo'):
                type_operation = self.get_type_operation(rc.location_id.id, 'entrepot')
                if type_operation:
                    picking = self.get_picking_id(type_operation, 'entrepot')
                    self.env['stock.move'].create({
                        'product_id'      : rc.product_id.id,
                        'name'            : rc.product_id.name,
                        'origin'          : self.demande_id.name,
