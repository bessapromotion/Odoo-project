...
class PurchaseOrder(models.Model):
    ...
    total_discount_amount = fields.Monetary(string=' Total Remise', compute='_compute_total_discount_amount',
                                            digits='Product Price', default=0, store=True)
    total_amount_initial = fields.Monetary(string='Montant HT avant Remise', compute='_compute_total_discount_amount',
                                           digits='Product Price', default=0, store=True)

    @api.onchange('order_line', 'amount_total')
    def _compute_total_discount_amount(self):
        print('order')
        for rec in self:
            for order_line in rec.order_line:
                if order_line.discount != 0:
                    rec.total_discount_amount += order_line.discount_amount
                rec.total_amount_initial += order_line.sub_total_amount_initial
                print(rec.total_discount_amount)
                print(rec.total_amount_initial)
...
class PurchaseOrderLine(models.Model):
    ...
    discount = fields.Float(string="Remise (%)", digits=(12, 6), store=True)
    discount_amount = fields.Monetary(string='Remise (DA)', compute='_compute_discount_amount',
                                      digits='Product Price', default=0, store=True)
    sub_total_amount_initial = fields.Monetary(string='Montant avant Remise', compute='_compute_discount_amount',
                                               digits='Product Price', default=0, store=True)
    ...
    def _compute_discount_amount(self):
        for line in self:
            # if line.discount:
            print(line.discount)
            line.sub_total_amount_initial = line.product_qty * line.price_unit
            print(line.sub_total_amount_initial)
            line.discount_amount = (line.sub_total_amount_initial * line.discount) / 100
