...
class TransfertFond(models.Model):
    _name = 'transfert.fond'
    _description = u'Transfert Fond'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _order = 'name desc'

    name = fields.Char('Numero', readonly=1)
    order_id_source = fields.Many2one('sale.order', string=u'Bon de commande source', required=True)

    echeance_ids_source = fields.one2many(related='order_id_source.echeancier_ids', string='échéance source',
                                          readonly=1,
                                          store=True)
    order_id_destination = fields.Many2one('sale.order', string=u'Bon de commande destination', required=True)
    echeance_ids_destination = fields.one2many(related='order_id_destination.echeancier_ids',
                                               string='échéance destination', readonly=1,
                                               store=True)
