...
class RemiseCles(models.Model):
    ...
    state = fields.Selection([('draft', 'Nouveau'),
                              ('valid', u'Validée'), ('cancel', 'Annulé'), ('livred', 'Livrée')], string='Etat', default='draft',
                             tracking=True)
    product_ids = fields.One2many(related='reservation_id.product_ids', string='APPARTEMENT', readonly=1,
                                  states={'draft': [('readonly', False)]})
    decision_id = fields.Many2one(related='reservation_id.decision_id', string='Décision', readonly=1,
                                  states={'draft': [('readonly', False)]})
    ...
    num_dossier = fields.Char(related='order_id.num_dossier', string=u'N° Dossier', store=True,
                              tracking=True)
    partner_id = fields.Many2one(related='reservation_id.partner_id', string=u'Réservataire', readonly=1, store=True)
    acquereur_id = fields.Many2one('product.partner', string=u"Propriétaire",
    ...
    date_livraison = fields.Date('Date de la livraison', states={'valid': [('required', True)],'livre': [('readonly', True)],}, store=True, default=lambda self: fields.Date.today())
    @api.model
    ...
    def action_validate(self):
        ...
        #    raise UserError(_(u'Veuillez creer la décision d\'affectation avant la remise des clés !' ))
        decision = self.decision_id
        if decision:
            if decision.signataire:
                decision.new_partner_id.etat = u'Acquéreur'
            else:
                self.partner_id.etat = u'Acquéreur'
        self.state = 'valid'
        self.order_id.state = 'done'
        self.reservation_id.remise_id = self.id
    ...
    def action_livraison(self):
        if self.date_livraison:
            ...
            raise UserError(_(u'Veuillez renseigner la date de la livraison !'))
    ...
    def action_print(self):
        ...
        if self.company_id != self.env.company:
            ...
            if self.type_bien == 'Appartement':
                return self.env.ref('crm_remise_cles.act_report_pv_remise_appartement').report_action(self)
            if self.type_bien == 'Cellier':
                return self.env.ref('crm_remise_cles.act_report_pv_remise_cellier').report_action(self)
    ...
    def print_cahier_charge(self):
        ...
        # print(data)
        return self.env.ref('crm_remise_cles.act_report_cahier_charge').report_action(self)
