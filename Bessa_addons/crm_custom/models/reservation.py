...
class CrmReservation(models.Model):
    ...
    discount_state = fields.Boolean(related='order_id.discount_state',store=True, default=False, readonly=True)
    new_price_m2 = fields.Float(related='order_id.new_price_m2',tracking=True, store=1, readonly=True)
    type = fields.Selection(related='order_id.type', string='Type de remise', readonly=True)
    ...
    def action_cancel(self, motif, annuler_echeance=True):
        ...
        for rec in self.product_ids:
            ...
            product_partner_id = self.env['product.partner'].search([('name', '=', rec.name.client_id.id),
                                                                     ('product_id', '=', rec.name.id)])
            product_partner_id.unlink()
