...
class CaculRemisePourcentageWizard(models.TransientModel):
    ...
    def action_appliquer(self):
        if self.discount  :
            ecart = self.prix_m2_actuel - self.nouveau_prix_m2
            user_id = self.env.user.id
            print('user_id',user_id)
            if ecart > 5000 and user_id not in (68,61):
                raise UserError(_(
                    u'Le montant de la remise doit etre inférieur au seuil qui est 5000 Da'))
            ...
            if self.order_id.nbr_echeances > 0:
                self.env['crm.echeancier'].create({
                    'name': '#Remise',
                    'order_id': self.order_id.id,
                    'label': 'Ajustement des échéances suite à l\'application d\'une remise #' ,
                    'date_creation': date.today(),
                    'date_prevue': date.today(),
                    'montant_prevu':   self.nouveau_prix_vente - self.prix_vente_actuel,
                    'type': 'remboursement',
                    'montant': 0.0,
                })
