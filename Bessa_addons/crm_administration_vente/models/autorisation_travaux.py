...
class AutorisationTravaux(models.Model):
    _name = 'autorisation.travaux'
    _description = u'Autorisation de travaux '
    ...
    detail_travaux = fields.Html(string=u'Détail des travaux')
    delai = fields.Integer(string=u'Nombre de jours')
    avis = fields.Selection([('favorable', 'Favorable'),
                             ('défavorable', u'Défavorable'), ('reserve', u'Défavorable avec Réserve')],
                            string='Avis du promoteur', store=True, tracking=True)
    reserves = fields.Html(u'Réserves')

    @api.model
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].get('autorisation.travaux') or '/'

        return super(AutorisationTravaux, self).create(vals)
    ...
    def unlink(self):
        if self.state != 'draft':
            ...
            rec = super(AutorisationTravaux, self).unlink()
            return rec
        ...
        if self.reservation_id.decision_id :
           if self.reservation_id.decision_id.state != 'valid' :
               ...
               if self.reservation_id.remise_id:
                   if self.reservation_id.remise_id.state == 'draft':
    ...
    def action_print(self):
        if self.company_id != self.env.company:
            ...
            self.ensure_one()
            return self.env.ref('crm_administration_vente.act_report_autorisation_travaux').report_action(self)
